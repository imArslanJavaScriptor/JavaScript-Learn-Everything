...
// forEach: Useful for iterating over the array to perform side effects (like console.log or saving to a database). It always returns undefined.

// map: Iterates over an array and returns a new array of the same length, where every element has been transformed by your function.

// filter: (This is where your note was swapped!) Iterates over an array and returns a new array containing only the elements that pass a test (condition).

// reduce: Iterates over the array to "boil down" all elements into a single value (like a sum, a string, or an object) using an accumulator.